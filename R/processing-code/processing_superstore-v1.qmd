...
The purpose of this project is to group customers based on their purchase pattern. It uses a transnational data set 'superstoredata' spanning from December 1, 2010, to December 9, 2011 from a UK-based, non-store online retail operation. The company specializes in unique all-occasion gifts to a clientele which also includes wholesalers.

The project focuses on customers residing outside of UK.In the first step of cleaning, all sales in the UK was excluded. The data set data_without_uk excludes all sales in the UK. It has 46431 observations with 9 variables. It is important to note that this data set accounts sales of a particular item as an observation. This means different items sold in the same Invoice appears as separate observations and it is most likely that an Invoice number could appear in separate observations. 
